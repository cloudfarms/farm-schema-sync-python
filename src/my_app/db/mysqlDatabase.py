...
class MySqlDatabase(BaseDatabase[ServerDbConfig]):

    def __init__(self,config:ServerDbConfig)->None:
        self.conn = mysql.connect(host=config.dbHost, port=config.dbPort, database=config.dbName, user=config.dbUser, password=config.dbPassword)
