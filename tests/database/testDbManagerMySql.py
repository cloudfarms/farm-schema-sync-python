...
class TestDbManagerMySql(unittest.IsolatedAsyncioTestCase):
    ...
    def setUp(self):
        ...
        try:
            ...
            self.mgr.cursor.execute("SET FOREIGN_KEY_CHECKS = 1")
    ...
    def testSetUp(self):
        ...
        self.assertEqual(self.mgr.dialect, Dialect.MYSQL)
        self.assertIsInstance(self.mgr.generator, MySqlGenerator)
        self.assertIsInstance(self.mgr.conn, MySQLConnectionAbstract)
        self.assertIsInstance(self.mgr.cursor, MySQLCursorAbstract)
    ...
    def testExecSchema(self):
        ...
        tableExistQuery = (f"SHOW TABLES LIKE '{EMPLOYEES_TABLE.name}'")
        colDataQuery = (f"DESCRIBE {EMPLOYEES_TABLE.name}")
        try:
