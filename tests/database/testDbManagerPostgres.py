...
class TestDbManagerPostgres(unittest.IsolatedAsyncioTestCase):
    ...
    def setUp(self):
        ...
        self.config = ServerDbConfig(
            ...
            dbPort = int(os.getenv("POSTGRES_DB_PORT")),
            dbUser = os.getenv("POSTGRES_DB_USER"),
            dbPassword = os.getenv("POSTGRES_DB_PASSWORD")
        )
        self.mgr = DbManager(self.config, Dialect.POSTGRES)
        ...
        try:
            ...
            if tables:
                table_names_str = ", ".join([f'"{row[0]}"' for row in tables])
                drop_query = f"DROP TABLE IF EXISTS {table_names_str} CASCADE;"
    ...
    def testSetUp(self):
        ...
        self.assertEqual(self.mgr.dialect, Dialect.POSTGRES)
        self.assertIsInstance(self.mgr.generator, PostgresGenerator)
        self.assertIsInstance(self.mgr.conn, psycopg2.extensions.connection)
        self.assertIsInstance(self.mgr.cursor, psycopg2.extensions.cursor)
    ...
    def testExecSchema(self):
        ...
        tableExistQuery = ("SELECT table_name FROM information_schema.tables WHERE"
                           f" table_name = '{EMPLOYEES_TABLE.name}'")
        colDataQuery = ("SELECT column_name FROM information_schema.columns"
        f" WHERE table_name = '{EMPLOYEES_TABLE.name}'")
        try:
    ...
    def testReadHoldingIds(self):
        ...
        try:
            self.mgr.cursor.execute("CREATE TABLE IF NOT EXISTS holding"
                                    " (id INT PRIMARY KEY, name TEXT, parent_id INT,"
    ...
    def testReadFarmIds(self):
        ...
        try:
            self.mgr.cursor.execute("CREATE TABLE IF NOT EXISTS farm"
                                    " (id INT PRIMARY KEY, name TEXT, holding_id INT,"
